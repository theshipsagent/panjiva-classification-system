"""
Transform USACE Entrance/Clearance Data v2.0.0
...
def transform_entrance_data(input_file, output_file, test_mode=False):
    ...
    print("=" * 80)
    print("USACE Entrance Data Transformation v2.0.0")
    print("=" * 80)
    print()
    ...
    # Map Arrival Port (PORT column)
    print("  Mapping Arrival Port (PORT)...")
    df['US_Port_USACE'] = df['PORT'].apply(
        lambda x: usace_port_lookup.get(str(x), '') if pd.notna(x) and x != '' else ''
    )
    matched_arrival = len(df[df['US_Port_USACE'] != ''])
    print(f"    Matched: {matched_arrival}/{len(df)} ({matched_arrival/len(df)*100:.1f}%)")
    ...
    columns_to_keep = [
        ...
        'Count',
        'TYPEDOC',
        'Arrival_Date',  # was ECDATE

        # Arrival Port (US)
        'PORT',
        'Arrival_Port_Name',  # was PORT_NAME
    ...
    arrival_mapped = len(df_final[df_final['US_Port_USACE'] != ''])
    ...
    print(f"  Arrival Port (USACE):        {arrival_mapped:,} / {len(df_final):,} ({arrival_mapped/len(df_final)*100:.1f}%)")
    ...
    for idx in range(min(3, len(df_final))):
        ...
        print(f"  Arrival: {rec['Arrival_Port_Name']}")
        print(f"  Mapped:  {rec['US_Port_USACE']}")
        if rec['Draft_Pct_of_Max']:
            print(f"  Draft Analysis: Actual {rec['DRAFT_FT']}ft {rec['DRAFT_IN']}in, {rec['Draft_Pct_of_Max']}% of max -> {rec['Forecasted_Activity']}")
...
def main():
    ...
    OUTPUT_FILE = Path(r"G:\My Drive\LLM\project_manifest\02_STAGE02_CLASSIFICATION\usace_2023_inbound_entrance_transformed_v2.1.0.csv")
    ...
    print("\n\n")
    print("#" * 80)
    print("# RUNNING FULL DATASET - All records")
    print("#" * 80)
    print("\n")

    df_full = transform_entrance_data(INPUT_FILE, OUTPUT_FILE, test_mode=False)
