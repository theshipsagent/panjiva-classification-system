"""
Classification Test for v2.0.0 Preprocessed Data
=================================================

Tests classification on 5K preprocessed v2.0.0 sample.

Changes from previous versions:
- NO vessel enrichment (already in preprocessing)
- NO column initialization (already in preprocessing)
- ONLY classification logic

Input: Preprocessed v2.0.0 CSV (59 columns, all initialized)
Output: Classified CSV (59 columns, Group/Commodity/Cargo/Cargo Detail filled)
...
# Paths
INPUT_FILE = Path(r"G:\My Drive\LLM\project_manifest\00_DATA\00.02_PREPROCESSED\sample_5000_preprocessed_v2.0.0_20260128_121323.csv")
...
OUTPUT_FILE = OUTPUT_DIR / "sample_5k_classified_v2.0.0_test.csv"
STATS_FILE = OUTPUT_DIR / "classification_stats_v2.0.0_5k_test.csv"
...
def load_data():
    ...
    stamp(f"Vessel types populated: {(df['Vessel_Type_Simple'].notna() & (df['Vessel_Type_Simple'] != '')).sum()} / {len(df)}")
...
def main():
    ...
    try:
        stamp("\n" + "="*80)
        stamp("CLASSIFICATION TEST v2.0.0")
        stamp("="*80)
