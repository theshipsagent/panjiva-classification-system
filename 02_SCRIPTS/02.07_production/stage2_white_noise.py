...
Input:  panjiva_ALL_YEARS_stage1.csv
Output: panjiva_ALL_YEARS_stage2.csv
...
INPUT_FILE = BASE_DIR / "00_DATA" / "00.03_MATCHED" / "panjiva_ALL_YEARS_stage1.csv"
OUTPUT_FILE = BASE_DIR / "00_DATA" / "00.03_MATCHED" / "panjiva_ALL_YEARS_stage2.csv"
...
def exclude_white_noise():
    ...
    stamp("="*80)
    stamp("STAGE 2: WHITE NOISE FILTER")
    stamp("="*80)
    start = datetime.now()
    ...
    stamp(f"\n" + "="*80)
    stamp(f"STAGE 2 COMPLETE - Duration: {duration:.1f} minutes")
    stamp("="*80)
