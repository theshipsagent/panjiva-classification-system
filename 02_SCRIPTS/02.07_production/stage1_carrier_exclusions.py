...
Input:  panjiva_ALL_YEARS_preprocessed.csv (clean, no classification)
Output: panjiva_ALL_YEARS_stage1.csv
...
EXCLUSION_LIST = BASE_DIR / "01_DICTIONARIES" / "01.04_carriers" / "carrier_exclusion_list.csv"
INPUT_FILE = BASE_DIR / "00_DATA" / "00.02_PREPROCESSED" / "panjiva_ALL_YEARS_preprocessed.csv"
OUTPUT_FILE = BASE_DIR / "00_DATA" / "00.03_MATCHED" / "panjiva_ALL_YEARS_stage1.csv"
...
def apply_carrier_exclusions():
    ...
    stamp("="*80)
    stamp("STAGE 1: CARRIER EXCLUSIONS")
    stamp("="*80)
    start = datetime.now()
    ...
    stamp(f"\n" + "="*80)
    stamp(f"STAGE 1 COMPLETE - Duration: {duration:.1f} minutes")
    stamp("="*80)
