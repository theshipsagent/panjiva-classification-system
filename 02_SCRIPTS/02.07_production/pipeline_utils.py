"""
Pipeline Utilities - Shared Functions for Classification Stages
================================================================
Common functions used across stage1-stage7 scripts.

Eliminates code duplication and ensures consistent behavior.

Functions:
- stamp(): Timestamped logging
- load_stage_data(): Load CSV with standard preprocessing
- apply_classification(): Apply rule to matching records
- print_stage_results(): Standard results section
...
# Standard paths
DATA_PREPROCESSED = BASE_DIR / "00_DATA" / "00.02_PREPROCESSED"
DATA_MATCHED = BASE_DIR / "00_DATA" / "00.03_MATCHED"
DICTIONARIES = BASE_DIR / "01_DICTIONARIES"
...
def stamp(msg: str):
    ...
    Used consistently across all stage scripts for uniform output.
...
def load_stage_data(
    ...
    """
    Load stage data with standard preprocessing.
    ...
    Example:
        df, stats = load_stage_data(INPUT_FILE)
...
def save_stage_data(df: pd.DataFrame, output_file: Path, drop_temp_cols: bool = True):
    """
    Save stage data with cleanup.
...
def print_stage_results(
    ...
    """
    Print standard results section for a stage.

    Args:
        df: DataFrame after processing
        stage_name: Name of stage (e.g., "Stage 4")
        stats_before: Statistics dict from before processing
        captured_records: Records classified in this stage
        captured_tons: Tons classified in this stage
        by_category: Optional breakdown by commodity/cargo
    """
    stamp("\n" + "=" * 80)
    stamp(f"RESULTS - {stage_name}")
    stamp("=" * 80)
...
class StageTimer:
    """
    Simple timer for tracking stage duration.

    Example:
        timer = StageTimer("Stage 4")
        # ... processing ...
        timer.complete()
    """
    ...
    def complete(self, records_processed: int = None):
        ...
        stamp(f"\n{'=' * 80}")
        stamp(f"{self.stage_name} COMPLETE")
...
def get_monthly_files(year: int, month: int, stage: int = None) -> Tuple[Path, Path]:
    """
    Get input/output paths for monthly batch processing.
    ...
    if stage == 1:
        # Stage 1 reads from preprocessed
        input_file = DATA_PREPROCESSED / f"panjiva_{year}_{month_str}_preprocessed.csv"
    else:
        # Other stages read from previous stage output
        input_file = DATA_MATCHED / f"panjiva_{year}_{month_str}_stage{stage-1}.csv"

    output_file = DATA_MATCHED / f"panjiva_{year}_{month_str}_stage{stage}.csv"
