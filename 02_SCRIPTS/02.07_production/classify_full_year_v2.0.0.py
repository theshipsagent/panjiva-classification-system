"""
Full Year Classification for v2.0.0 Data
=========================================

Classifies entire year of preprocessed v2.0.0 data using dictionary v3.6.0.
...
Expected runtime: 8-12 hours per year (depends on record count)
...
DICTIONARY = BASE_DIR / "01_DICTIONARIES" / "01.01_cargo_classification" / "cargo_classification_dictionary_CURRENT_v3.6.0.csv"
...
def load_data(year):
    """Load full year of preprocessed v2.0.0 data"""
    ...
    if not input_file.exists():
        stamp(f"ERROR: File not found: {input_file}")
        sys.exit(1)
...
def load_dictionary():
    ...
    if not DICTIONARY.exists():
        stamp(f"ERROR: Dictionary not found: {DICTIONARY}")
        sys.exit(1)
    ...
    for phase in phases:
        ...
        for idx in df.index:
            ...
            if (idx + 1) % progress_interval == 0:
                ...
                stamp(f"    Progress: {idx+1:,} / {total_records:,} ({pct:.1f}%) - {phase_matches:,} matches so far")
        ...
        stamp(f"  Matched: {phase_matches:,} records in {phase_duration:.1f} seconds")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Classify full year of v2.0.0 data')
    parser.add_argument('--year', type=int, required=True, choices=[2023, 2024, 2025],
                        help='Year to classify (2023, 2024, or 2025)')
    args = parser.parse_args()

    year = args.year

    # Define output files
    output_file = OUTPUT_DIR / f"panjiva_imports_{year}_classified_v2.0.0.csv"
    stats_file = OUTPUT_DIR / f"classification_stats_{year}_v2.0.0.csv"

    try:
        stamp("\n" + "="*80)
        stamp(f"FULL YEAR CLASSIFICATION - {year} v2.0.0")
        stamp("="*80)
        start_time = datetime.now()
        stamp(f"Start time: {start_time}")
        ...
        stamp("="*80)
        stamp(f"Start time:    {start_time}")
        stamp(f"End time:      {end_time}")
        stamp(f"Duration:      {duration:.2f} hours")
        stamp(f"Records:       {len(df):,}")
        stamp(f"Classified:    {(df['Group'].notna() & (df['Group'] != '')).sum():,}")
