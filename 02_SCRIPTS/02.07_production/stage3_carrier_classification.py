"""
Stage 3: Carrier Classification (RoRo/Reefer)
==============================================
Classify RoRo and Reefer carriers based on carrier SCAC.

Input:  panjiva_ALL_YEARS_stage2.csv
Output: panjiva_ALL_YEARS_stage3.csv
...
CARRIER_RULES = BASE_DIR / "01_DICTIONARIES" / "01.04_carriers" / "carrier_classification_rules.csv"
INPUT_FILE = BASE_DIR / "00_DATA" / "00.03_MATCHED" / "panjiva_ALL_YEARS_stage2.csv"
OUTPUT_FILE = BASE_DIR / "00_DATA" / "00.03_MATCHED" / "panjiva_ALL_YEARS_stage3.csv"
...
def apply_carrier_classification():
    ...
    stamp("="*80)
    stamp("STAGE 3: CARRIER CLASSIFICATION (RoRo/Reefer)")
    stamp("="*80)
    start = datetime.now()
    ...
    stamp(f"\n" + "="*80)
    stamp(f"STAGE 3 COMPLETE - Duration: {duration:.1f} minutes")
    stamp("="*80)
