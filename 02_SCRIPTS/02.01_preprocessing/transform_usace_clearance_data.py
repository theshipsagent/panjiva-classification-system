"""
Transform USACE Clearance Data v2.2.0 (Outbound/Exports)
...
Updates in v2.1.0:
- Added Port_Consolidated, Port_Coast, Port_Region from US port dictionary
- Added Tons column (placeholder)
- Added Carrier_Name column (placeholder)
- Added Agency_Fee column (matched from ICST_DESC)
- Added Agency_Fee_Adj column (placeholder)
...
def transform_clearance_data(input_file, output_file, test_mode=False):
    """Transform USACE clearance data"""

    print("=" * 80)
    print("USACE Clearance Data Transformation v2.1.0 (Outbound/Exports)")
    print("=" * 80)
    print()
    ...
    # USACE-to-Census Port Mapping (for Port_Consolidated, Port_Coast, Port_Region)
    ...
    for _, row in df_port_mapping.iterrows():
        ...
        if row['Match_Type'] != 'NO MATCH':
            port_stats_lookup[usace_port] = {
                ...
                'Census_Sked_D_E': str(row.get('Census_Sked_D_E', '')).strip()
            }
    print(f"  Loaded {len(port_stats_lookup)} USACE port statistical category mappings (44.3% coverage)")

    # Foreign ports dictionary
    ...
    print("  Mapping Clearance Port (USACE codes)...")
    ...
    # Map US Port Statistical Categories (PORT column via USACE-to-Census mapping)
    print("  Mapping US Port Statistical Categories...")
    df['Census_Code'] = df['PORT'].apply(
        lambda x: port_stats_lookup.get(str(x), {}).get('Census_Code', '') if pd.notna(x) and x != '' else ''
    )
    df['Census_Port_Name'] = df['PORT'].apply(
        lambda x: port_stats_lookup.get(str(x), {}).get('Census_Sked_D_E', '') if pd.notna(x) and x != '' else ''
    )
    df['Port_Consolidated'] = df['PORT'].apply(
        lambda x: port_stats_lookup.get(str(x), {}).get('Port_Consolidated', '') if pd.notna(x) and x != '' else ''
    )
    df['Port_Coast'] = df['PORT'].apply(
        lambda x: port_stats_lookup.get(str(x), {}).get('Port_Coast', '') if pd.notna(x) and x != '' else ''
    )
    df['Port_Region'] = df['PORT'].apply(
        lambda x: port_stats_lookup.get(str(x), {}).get('Port_Region', '') if pd.notna(x) and x != '' else ''
    )
    ...
    print(f"  Clearance Port (USACE):      {usace_mapped:,} / {len(df_final):,} ({usace_mapped/len(df_final)*100:.1f}%)")
...
def main():
    ...
    OUTPUT_FILE = Path(r"G:\My Drive\LLM\project_manifest\00_DATA/00.03_MATCHED\usace_2023_outbound_clearance_transformed_v2.2.0.csv")
    ...
    df_final = transform_clearance_data(INPUT_FILE, OUTPUT_FILE, test_mode=False)
